...
def update_dist(dist, previous, edges, s1, s2, MIGRATION_COST, WEIGHT_MIGRATION, WEIGHT_OVERLOAD):
	"""
...
def djikstra(vertices, edges, start, MIGRATION_COST, WEIGHT_MIGRATION, WEIGHT_OVERLOAD):
	dist, previous, nbVertices, nodes, edges, probabilities = init(vertices, edges, MIGRATION_COST, WEIGHT_MIGRATION, WEIGHT_OVERLOAD)
	...
	while len(q) > 0:
		...
		for s2 in range(nbVertices):
			if vertices[s1][s2] == 1:
				dist, previous = update_dist(dist, previous, edges, s1, s2, MIGRATION_COST, WEIGHT_MIGRATION, WEIGHT_OVERLOAD)
	return previous, probabilities

def shortest(edges, previous, start, target, MIGRATION_COST, WEIGHT_MIGRATION, WEIGHT_OVERLOAD, probabilities):
	path = []
...
def init(vertices, edges, MIGRATION_COST, WEIGHT_MIGRATION, WEIGHT_OVERLOAD):
	dist = []
	previous = []
	nodes = []
	...
	for i in range(nbVertices):
		...
		for j in range(nbVertices):
			if edges[i][j] == MIGRATION_COST:
				edges[i][j] = WEIGHT_MIGRATION * edges[i][j]
